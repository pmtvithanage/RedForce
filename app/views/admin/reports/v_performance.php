...
<script>
    ...
    function buildStatusChart(rows) {
        ...
        const dataValues = [counts.Present, counts.Absent];
    ...
    async function downloadPdfReport() {
        ...
        try {
            ...
            pdf.text('OFFICER PERFORMANCE', pageWidth / 2, 35, {
                align: 'center'
            });
            pdf.setFontSize(16);
            pdf.setFont(undefined, 'normal');
            pdf.text('Report & Analytics', pageWidth / 2, 50, {
                align: 'center'
            });
            pdf.setFontSize(11);
            pdf.text(`Generated: ${new Date().toLocaleString()}`, pageWidth / 2, 65, {
                align: 'center'
            });
            ...
            pdf.setFontSize(10);
            pdf.setFont(undefined, 'normal');
            pdf.text(`Officers in report: ${officerSummary.length}`, 25, yPos);
            yPos += 6;
            pdf.text(`Attendance records: ${filteredRows.length}`, 25, yPos);
            yPos += 6;
