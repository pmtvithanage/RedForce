...
<!-- Tabs -->
<div class="tabs">
    <button class="tab primary-btn" onclick="window.location.href='<?php echo URL_ROOT; ?>/admin/porecruitment'">Premise Officers</button>
...
<div class="form-container">
    ...
    <form class="job-form" action="<?php echo URL_ROOT; ?>/admin/porecruitment" method="post">
        ...
        <div class="form-actions">
            <?php if (isset($data['completed']) && $data['completed'] == 'true' && isset($data['status']) && $data['status'] == 'closed'): ?>
                <button class="primary-btn" style="margin-right: 45%;" onclick="window.location.href='<?php echo URL_ROOT; ?>/admin/changeStatus/po/open'">Open Application</button>
            <?php endif; ?>

            <?php if (isset($data['completed']) && $data['completed'] == 'true' && isset($data['status']) && $data['status'] == 'open'): ?>
                <button class="primary-btn" style="margin-right: 45%;" onclick="window.location.href='<?php echo URL_ROOT; ?>/admin/changeStatus/po/closed'">Close Application</button>
            <?php endif; ?>
            <button type="submit" class="primary-btn" name="submit">Save</button>
            <button class="secondary-btn" type="reset" onclick="window.location.href='<?php echo URL_ROOT; ?>/admin/delete_job_application/po/'">Clear</button>
        </div>
    </form>
</div>
