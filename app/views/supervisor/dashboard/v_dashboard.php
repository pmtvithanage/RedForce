<?php require_once APP_ROOT . '/views/inc/components/header.php'; ?>
<?php require_once APP_ROOT . '/views/components/v_supervisor_sidebar.php'; ?>
<link rel="stylesheet" href="<?= URL_ROOT ?>/css/admin/dashboard_style.css">

<div class="dashboard">
<!-- Stats -->
<div class="card stat-card">
  <span class="material-symbols-outlined stat-icon">group</span>
  <div>
    <div class="stat-value"><?php echo $data['attendanceStats']['total'] ?? 0; ?></div>
    <div>Total Officers</div>
  </div>
</div>

<div class="card stat-card">
  <span class="material-symbols-outlined stat-icon">shield</span>
  <div>
    <div class="stat-value"><?php echo $data['attendanceStats']['present'] ?? 0; ?></div>
    <div>On Duty</div>
  </div>
</div>

<div class="card stat-card">
  <span class="material-symbols-outlined stat-icon">verified</span>
  <div>
    <div class="stat-value"><?php echo ($data['attendanceStats']['total'] ?? 0) - ($data['attendanceStats']['absent'] ?? 0); ?></div>
    <div>Active</div>
  </div>
</div>

<div class="card stat-card">
  <span class="material-symbols-outlined stat-icon">error</span>
  <div>
    <div class="stat-value">0</div>
    <div>Incidents</div>
  </div>
</div>

<!-- Recent Activity -->
<div class="card section recent">
    <h3>Recent Activities</h3>
    
    <?php if (!empty($data['recent_activities'])): ?>
      <div class="activity-list">
        <?php foreach ($data['recent_activities'] as $activity): ?>
          <div class="activity-item <?php echo $activity->activity_type ?? ''; ?>">
            <div class="activity-header">
              <span class="material-symbols-outlined activity-icon">
                <?php 
                $icon_map = [
                  'attendance' => 'how_to_reg',
                  'leave' => 'event_busy',
                  'site' => 'location_on',
                  'officer' => 'badge',
                  'shift' => 'schedule',
                  'task' => 'task_alt',
                  'message' => 'mail',
                  'report' => 'description'
                ];
                echo $icon_map[$activity->activity_type ?? ''] ?? 'notifications';
                ?>
              </span>
              <div class="activity-title">
                <strong><?php echo htmlspecialchars($activity->activity_titel ?? 'Activity'); ?></strong>
              </div>
            </div>
            <div class="activity-details">
              <p><?php echo htmlspecialchars($activity->activity_details ?? ''); ?></p>
            </div>
            <?php if (!empty($activity->created_at)): ?>
              <div class="activity-user">
                <small><?php echo date('M d, Y   |   h:i A', strtotime($activity->created_at)); ?></small>
              </div>
            <?php endif; ?>
          </div>
        <?php endforeach; ?>
      </div>
    <?php else: ?>
      <div class="empty-activity">
        <span class="material-symbols-outlined">inbox</span>
        <p>No recent activity to display.</p>
        <small>Your team activities will appear here.</small>
      </div>
    <?php endif; ?>
  </div>

  <!-- Advertisements -->
  <div class="card section">
    <h3>Advertisements</h3>
    
    <?php if (!empty($data['advertisements'])): ?>
      <div class="ad-list">
        <?php foreach ($data['advertisements'] as $ad): ?>
          <div class="ad-item">
            <?php if (!empty($ad->image_path)): ?>
              <div class="ad-image">
                <img src="<?php echo URL_ROOT; ?>/<?php echo htmlspecialchars($ad->image_path); ?>" alt="Advertisement">
              </div>
            <?php endif; ?>
            <div class="ad-content">
              <h4><?php echo htmlspecialchars($ad->title); ?></h4>
              <?php if (!empty($ad->created_at)): ?>
                <small><?php echo date('M d, Y', strtotime($ad->created_at)); ?></small>
              <?php endif; ?>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
    <?php else: ?>
      <div class="empty-activity">
        <span class="material-symbols-outlined">campaign</span>
        <p>No advertisements available.</p>
        <small>New updates will appear here.</small>
      </div>
    <?php endif; ?>
  </div>

</div>


<script src="<?php echo URL_ROOT; ?>/js/components/sidebar.js"></script>
<?php require_once APP_ROOT . '/views/inc/components/footer.php'; ?>
