...
class M_admin
{
            ...
            SELECT 
                ...
                (SELECT COUNT(DISTINCT osa.officer_id) 
                 ...
                 WHERE (
                     ...
                     OR s.client_id = (SELECT user_id FROM Clients WHERE id = :client_id)
                     OR s.client_id = (SELECT id FROM Clients WHERE user_id = :client_id)
                 )
    ...
    public function approvePackageRequest($id, $admin_id, $notes)
    {
        ...
        // Step 3: Align assignment windows with approved service period.
        $this->applyApprovedServicePeriodToAssignments($id);

        // Return the site ID (or true if site creation failed but approval succeeded)
        return $siteId ? $siteId : true;
    }

    private function resolveApprovedRequestSiteContext($requestId, $draftSiteId = null)
    {
        $this->db->query("SELECT id, package_request_id
                          FROM sites
                          WHERE package_request_id = :request_id
                          ORDER BY id DESC
                          LIMIT 1");
        $this->db->bind(':request_id', $requestId);
        $direct = $this->db->single();

        if ($direct) {
            return [
                'site_id' => (int)$direct->id,
                'is_direct_site' => true
            ];
        }

        if (!empty($draftSiteId)) {
            $site = $this->getSiteById((int)$draftSiteId);
            if ($site) {
                return [
                    'site_id' => (int)$site->id,
                    'is_direct_site' => ((int)($site->package_request_id ?? 0) === (int)$requestId)
                ];
            }
        }
    ...
    private function applyApprovedServicePeriodToAssignments($requestId)
    {
        $this->db->query("SELECT id, start_date, end_date, submitted_date, draft_site_id
        ...
        $context = $this->resolveApprovedRequestSiteContext($requestId, $request->draft_site_id ?? null);
        if (!$context || empty($context['site_id'])) {
            return true;
        }
        ...
        $submittedDate = !empty($request->submitted_date) ? $request->submitted_date : $startDate;

        if (!empty($context['is_direct_site'])) {
            $this->db->query("UPDATE officer_site_assignments
                              SET assignment_start = :start_date,
        ...
        $this->db->query("UPDATE officer_site_assignments
                          ...
                          WHERE site_id = :site_id
                            AND status = 'Active'
                            AND assigned_at >= :submitted_date");
        $this->db->bind(':start_date', $startDate);
        $this->db->bind(':end_date', $endDate);
        $this->db->bind(':site_id', $siteId);
        $this->db->bind(':submitted_date', $submittedDate);
        $this->db->execute();
    ...
    public function approvePackageRequestFinal($requestId, $adminId)
    {
        ...
        $this->applyApprovedServicePeriodToAssignments($requestId);
        return true;
    }
    ...
    public function getAllClientsPayments()
    {
        $this->db->query('
            SELECT 
                ...
                u.profile_image as client_logo,
                s.site_name,
