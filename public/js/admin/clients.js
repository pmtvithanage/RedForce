// clients.js - Admin Clients Management
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Utility function to get tab buttons
  function getTabButtons() {
    return Array.from(document.querySelectorAll("#siteModal .tab"));
  }
  ...
  function mockStaffForSite(site) {
    ...
    return {
      officers: makeStaff(10),
      supervisors: makeStaff(4),
      caretakers: makeStaff(3),
    };
  }

  // Render sites in client modal
  ...
  siteSearchEl.addEventListener("input", () => {
    ...
    const filtered = currentSites.filter(
      (s) =>
        s.code.toLowerCase().includes(q) || s.address.toLowerCase().includes(q)
    );
  ...
  modalEl.addEventListener("click", (e) => {
    if (e.target.hasAttribute("data-close")) closeClientModal();
  });

  closeSiteBtn.addEventListener("click", closeSiteModal);
  siteModalEl.addEventListener("click", (e) => {
    if (e.target.hasAttribute("data-close")) closeSiteModal();
  });
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && modalEl.classList.contains("open"))
      closeClientModal();
    if (e.key === "Escape" && siteModalEl.classList.contains("open"))
      closeSiteModal();
  });
